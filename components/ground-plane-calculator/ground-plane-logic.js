...
export function calculateGroundPlane(
  frequencyHz,
  velocityFactor = 0.95,
  ...
  const quarterWave = (wavelength / 4) * velocityFactor;
  const radialLength = quarterWave * radialCorrection(radialAngleDegrees);

  return {
    wavelength,
    radiator: quarterWave,
    radial: radialLength,
    radialCount: 4,
    radialAngleDegrees,
  };
...
function radialCorrection(angleDegrees) {
  const number = Number(angleDegrees);
  assertRange(number, 0, 90, "angulo dos radiais");
  return 1 + (number / 90) * 0.04;
}
